...
const SportsBets = ({ selected, setSelected }: { selected: string, setSelected: any }) => {

    return (
        <div className='mx-20'>
            <h1 className=" text-2xl capitalize ml-28 font-bold">Sports Bets</h1>
            <div>
                <div className=" flex flex-wrap items-center justify-center">
                    {events.map((event) => (
                        ...
                        >
                            ...
                            <h1 className={`absolute bottom-0 group-hover:blur-0  transition ease-in-out duration-500 group-hover:text-white translate-x-1 -translate-y-1/2 text-3xl text-slate-300 blur-[1px] capitalize font-bold  ${event.for === selected ? "blur-0" : ""}  `}>
                                {event.title}
                            </h1>
                        </div>
                    ))}
